...
def peat_trainer(args) :
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, num_workers=num_worker, drop_last=True)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size, num_workers=num_worker, drop_last=True)
    ...
    model = smp.Unet(encoder_name='resnet34', encoder_depth= args.encoder_depth, encoder_weights='imagenet', decoder_use_batchnorm=True, decoder_channels=(256, 128, 16), decoder_attention_type=None, in_channels=3, classes=1, activation=None, aux_params=None)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=25, threshold= 1e-3 )
    best_loss = float('inf')
    ...
    for epoch in range(num_epoch):
        ...
        # if val_loss < loss_bench :
        #     loss_bench = val_loss
        #     model_name = "{}_{:.2f}_{}".format(model_version,loss_bench, time.strftime("%d_%m"))
